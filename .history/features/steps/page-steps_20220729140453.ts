import { SingleStuffPage } from '../../src/pages/SingleStuffPage';
import { StuffPage } from '../../src/pages/StuffPage';
import { HomePage } from '../../src/pages/MainPage';
import { BagPage } from '../../src/pages/BagPage';
...
When('I change amount of stuff', {timeout: 100 * 1000}, async function(){
    const singlePage: SingleStuffPage = new SingleStuffPage(this.world.driver);
    await singlePage.increasingAmountStuff();
    await singlePage.decreasingAmountStuff();
});

Then('I add stuff to the basket', {timeout: 100 * 1000}, async function(){
    const singlePage: SingleStuffPage = new SingleStuffPage(this.world.driver);
    await singlePage.clickAddToBagButton();
    await singlePage.clickViewBagButton();
});

Given('I am on basket page', {timeout: 100 * 1000}, async function(){
    const bagPage: BagPage = new BagPage(this.world.driver);
    await bagPage.goToBagPage();
});

When('I click on checkout button', {timeout: 100 * 1000}, async function(){
    const bagPage: BagPage = new BagPage(this.world.driver);
    await bagPage.clickOnCheckoutButton();
});

Given('I am on checkout page', {timeout: 100 * 1000}, async function(){
    const checkout: CheckoutPage = new CheckoutPage(this.world.driver);
    await checkout.goToCheckoutPage();
});

When('I fill name, lastname, email in Shipping Info', {timeout: 100 * 1000}, async function(){
    const checkout: CheckoutPage = new CheckoutPage(this.world.driver);
...
Then('I click on Confirm button', {timeout: 100 * 1000}, async function(){
    const checkout: CheckoutPage = new CheckoutPage(this.world.driver);
    await checkout.clickOnCheckoutConfirmButton();
});

When('I get errors from Shipping card form', {timeout: 100 * 1000}, async function(){
    const checkout: CheckoutPage = new CheckoutPage(this.world.driver);
    await checkout.checkPaymentPanelErrorMessagesText();
});

Then('I fill Shipping card info', {timeout: 100 * 1000}, async function(){
    const checkout: CheckoutPage = new CheckoutPage(this.world.driver);
