...
Given('test_1',{timeout: 100 * 1000}, async function(){
    const tst = new Calculator();
    this.result = await tst.sum(3, 5)
});
...
When('I get DATA from API', {timeout: 100 * 1000}, async function() {
    const cityApi = new restcountriesApiClient(this.parameters)
