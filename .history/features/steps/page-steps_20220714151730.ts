...
import { CityApiClient } from '../api/interfaceApi';
...
When('I get DATA from API', {timeout: 100 * 1000}, async function(this:CityApiClient) {
    const request: CityApiClient = new CityApiClient(this)
    // const requestData = request.getApi()
    // this.requestData = request.getApi()
})

When('I fill city and state', {timeout: 100 * 1000}, async function(requestData){
    const checkout: CheckoutPage = new CheckoutPage(driver);
    this.requestData = requestData
    await checkout.fillingInCityAndState(requestData);
})
