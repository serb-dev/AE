...
When('Chose a {string} day', {timeout: 100 * 1000}, async function(another){
    const homepage: MinusrusPage = new MinusrusPage(this.world.driver)
    await homepage.chooseDate(another)
})

Then('Verify that intendent loss for {string} entity calculated correctly', {timeout: 100 * 1000}, async function(entity: string){
    const homepage: MinusrusPage = new MinusrusPage(this.world.driver)
