...
import { delay, getCurrentDay, dateChecking } from '../utils/utils'
...
export class MinusrusPage {
    ...
    private uaBtn: By = By.xpath('//a[@href="/ua"]')
    ...
    public async choosingLanguage(language: string) {
        ...
        case 'ENG':
            const btnLanguage = await this.driver.wait(until.elementLocated(this.engBtn), 10000)
            await btnLanguage.click()
            break

        case 'RU':
            const btnLangRu = await this.driver.wait(until.elementLocated(this.ruBtn), 10000)
            await btnLangRu.click()
            break

        case 'UA':
            const btnLangUA = await this.driver.wait(until.elementLocated(this.uaBtn), 10000)
            await btnLangUA.click()
            break

        default: throw new Error('No any language matches')
        }
    }
