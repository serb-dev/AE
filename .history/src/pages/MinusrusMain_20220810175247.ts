...
export class MinusrusPage {
    ...
    public async checkingLanguage(): Promise<void> {
        const element = await this.driver.wait(until.elementLocated(this.dateLabel), 10000)
    ...
    public async changeDate(expectedDate: string): Promise<void> {
        await delay(3000)
        logger.info('delay')
        const dateTitleElement = await this.driver.wait(until.elementLocated(this.logoText), 10000)
        logger.info('date')
