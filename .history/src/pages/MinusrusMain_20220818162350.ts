...
export class MinusrusPage {
    ...
    public async chooseDate(expectedDate: string): Promise<void> {
        ...
        const newExpectedDate = await this.changeStringToDATE(expectedDate)
        const newCurrentDate = await this.changeStringToDATE(currentDate)
    ...
    private async changeStringToDATE(date: string): Promise<string> {
