...
export class MinusrusPage {
    ...
    public async testTaskSwitcher(entity: string) {
        ...
        if(cardTitleText === 'Личный состав'){
            const dailyAmountElement = await baseElement.findElement(this.dailyAmount)
            const textAmountElement = await dailyAmountElement.getText()
            const slicedTextAmount = textAmountElement.slice(1, 8)
            textAmount = slicedTextAmount.replace('.', '')
        }else{
            const dailyAmountElement = await baseElement.findElement(this.dailyAmount)
            textAmount = await dailyAmountElement.getText()
        }
