...
export class MinusrusPage {
    ...
    private async calculationOfAircraftLosses(): Promise<void> {
        ...
        logger.info('get total number of aircraf losses:' + totalLossesAircraft)
        ...
        const aircraftLosses = this.calculatorOfPercent()
    }

    private async calculatorOfPercent(daily: string, intendet: string) {
        const percent =  +intendet / +daily
        logger.info(percent)
    }
}
