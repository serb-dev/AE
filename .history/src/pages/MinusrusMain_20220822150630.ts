...
export class MinusrusPage {
    ...
    private totalNumberOfEntityForces: By = By.className('statistics__description')
    ...
    public async testTaskSwitcher(entity: string) {
        ...
        const intendedForInvasionElements = await baseElement.findElements(this.intendetForInvasion)
        const intendedForInvasionElement = await intendedForInvasionElements[1].getText()
        const intendetForInvasionArray = intendedForInvasionElement.split('\n')
        const intendedForInvasionNumber = intendetForInvasionArray[0]
        const intendedForInvasionString = intendetForInvasionArray[1]

        logger.info(`${intendedForInvasionString}: ${intendedForInvasionNumber}`)
