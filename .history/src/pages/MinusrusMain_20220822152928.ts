...
export class MinusrusPage {
    ...
    public async testTaskSwitcher(entity: string) {
        ...
        const totalNumberOfForcesSplitedString = totalNumberOfForces.replace('.', '')

        logger.info(`${totalNumberOfForcesString}: ${totalNumberOfForcesSplitedString}`)

        const myCalculation = await this.calculatorOfPercent(textAmount, totalNumberOfForcesSplitedString)
