...
export class MinusrusPage {
    ...
    public async chooseDate(expectedDate: string): Promise<void> {
        ...
        const splitedExpectedDate = expectedDate.split('.')
        const reverrsedExpectedDate = splitedExpectedDate.reverse()
        const newExpectedDate = reverrsedExpectedDate.join('.')

        const splitedCurrentDate = currentDate.split('.')
        const reverrsedCurrentDate = splitedCurrentDate.reverse()
        const newCurrentDate = reverrsedCurrentDate.join('.')

        const currentDateToDATE = new Date(newCurrentDate)
        const expectedDateToDATE = new Date(newExpectedDate)

        while(expectedDate !== currentDate){
            if(expectedDateToDATE < currentDateToDATE){
                arrows[0].click()
