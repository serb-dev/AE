...
export class MinusrusPage {
    ...
    private cardTitle: By = By.xpath('//div[@class="card__title"]')
    ...
    public async testTaskSwitcher(entity: string) {
        ...
        const dailyAmountElement =  baseElement.findElement(this.dailyAmount)
        const textAmount = dailyAmountElement.getText()
        const cardTitleElement = baseElement.findElement(this.cardTitle)
        const cardTitleTxt = cardTitleElement.getText()
        logger.info(`${cardTitleTxt} : ${textAmount}`)
