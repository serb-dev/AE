...
export class MinusrusPage {
    ...
    public async changeDate(expectedDate: string): Promise<void> {
        ...
        const currentDateFn = function(driver = this.driver) {
            const dateElement: WebElement =  driver.wait(until.elementLocated(this.dateLabel), 10000)
            const date =  dateElement.getText()
            return date
        }
        const currentDate: string = currentDateFn().toString()
        ...
        // await delay(3000)
        // await this.dateSwitcher(expectedDate, currentDate, leftArrow)
    ...
    private async dateSwitcher(expectedDate: string, currentDate: string, leftArrow: WebElement): Promise<void> {
